package io.github.treesitter.jtreesitter.mlir;

import java.lang.foreign.*;

public final class TreeSitterMlir {
    ...
    private static final TreeSitterMlir INSTANCE = new TreeSitterMlir();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterMlir() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_mlir");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-mlir");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-mlir");
